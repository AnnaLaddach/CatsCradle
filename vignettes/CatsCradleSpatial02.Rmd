...
DimPlot(neighbourhoodSeurat, group.by = c("seurat_clusters"), cols = "polychrome", reduction = "umap")
```
...
DimPlot(neighbourhoodSeuratExpanded, group.by = c("seurat_clusters"), cols = "polychrome", reduction = "umap")
```
We can now add information on neighbourhood clusters to our original xenium object and visualise these on the tissue.
```{r}
...
```{r}
interactionResults$interactionsOnEdges
```

...
### CatsCradle: cells and genes

CatsCradle exploits the duality between cells and genes.  A standard
...
CatsCradle exploits the duality by transposing the matrix so that
...
### Spatial CatsCradle: neighbourhoods and cell types
...
A neighbourhood is simply a set of cells.  For our purposes, we will
...
neighbourhoods are more appropriate for investigating interactions involving 
diffusible ligands.

### Extended neighbour graphs
...
(extendedNeighbours[[1]]) is the graph of 1st degree neighbours, the second item
the graph of 2nd degree neighbours etc.
...
Given the above methods to create neighbourhoods we can go on to investigate 
both the cell type composition of these neighbourhoods and their gene expression 
profiles.
...
12_Pvalb to 18_pyramidal because neighbourhoods of cells of type 12_Pvalb are composed 
(across the dataset) of ~15% cells of type 18_pyramidal.  We do not see an arrow 
from 18_pyramidal to 12_Pvalb because neighbourhoods of cells of type 18 have only 3%

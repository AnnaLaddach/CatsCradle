...
value based on permutation testing where expression values for genes are permuted. N.B 
...
between cell centroids tend to depend on the types of the neighbouring
cells.  Accordingly, we have supplied functions to annotate edges with
their distances and cell types; compute estimates for edge-length
cutoffs based on cell type pair; plot the distributions of lengths
together with proposed cutoffs; and subset the edges based on
cutoffs.
...
```{r}
 annEdges =
    edgeLengthsByCellTypePairs(delaunayNeighbours,clusters,centroids)
 head(annEdges)
 ```

...
Is the expression of particular genes localised?  Who are the
immediate neighbours of a given cell?  Do certain types of cells tend
to co-locate?  Is the association of two cell types, (say, glia and
macrophages) statistically significant?  For any pair of neighbouring
cells, are they engaged ligand-repceptor interaction?  Can we classify
the different types of tissue neighbourhoods?  Are different
neighbourhood types characterised by the cell types in them?  The
genes expressed in them?  The ligand-receptor interactions taking
place in them?
...
dimension reduction using PCA, UMAP, tSNE; discovery a marker genes
...
Note how Pyramidal, Pvalb and Lamp5 neurons are located close together indicating
they are often found in the same neighbourhoods.
...
Clustering cell types using the Louvain algorithm achaeives only one cluster. 
...
Lets look at the celltype composition of these clusters.
```{r}
...
We can compute a p-value for the localisation of gene expression using
medianComplementPValue() which is described in the vignette
[CatsCradle](doc/CatsCradleQuickStart.html).
...
Here the p-value is limited by the default number of permutation
trials used by medianComplementPValue()
...
involving diffusable ligands and receptors. We also analyse the number
...
Here we perform interaction analysis:
```{r}
...
cell A - cell B and cell B - cell A.
```{r}
...
Make a heatmap of summed mean ligand/receptor interactions between clusters (sum of all ligand 
receptor interactions on A-B edges)/(total A-B edges)
```{r}
...
Create a Seurat object where each point represents an edge between cells, and spatial coordinates are the centroids of edges between cells. The "expression matrix" is the binarised presence/absence of an interaction (ligand receptor pair) on an edge. This is useful for visualising where ligand receptor interactions occur spatially.
```{r}

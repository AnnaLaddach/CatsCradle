...
This method is faster than permutation testing, however for legacy purposes, and 
user flexibility, we allow for the calculation of P values by comparison to 
randomised graphs. The function offers two methods of randomisation, controlled 
...
interactions between clusters. Specifically probability of a cell being positive 
for a gene is calculated using the hypergeometric distribution (R phyper 
function), with:
...
This analytical method to calculate p-values has a faster run time than the 

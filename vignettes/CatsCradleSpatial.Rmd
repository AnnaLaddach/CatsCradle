...
cellTypesPerCellTypeGraphFromCellMatrix(cellTypesPerCellTypeMatrixExtended, minWeight = 0.05, colors = colors)
```
...
We look at interactions on edges.  For conscision, we display the
...
```{r}
ligRecMatrix = makeLRInteractionHeatmap(ligandReceptorResults, clusters, colors = colors, labelClusterPairs = F)
```
